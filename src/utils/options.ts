const countries = () => [
  'Afghanistan',
  'Aland Islands',
  'Albania',
  'Algeria',
  'American Samoa',
  'Andorra',
  'Angola',
  'Anguilla',
  'Antarctica',
  'Antigua and Barbuda',
  'Argentina',
  'Armenia',
  'Aruba',
  'Australia',
  'Austria',
  'Azerbaijan',
  'Bahamas',
  'Bahrain',
  'Bangladesh',
  'Barbados',
  'Belgium',
  'Belize',
  'Benin',
  'Bermuda',
  'Bhutan',
  'Bolivia',
  'Bonaire, Sint Eustatius and Saba',
  'Bosnia and Herzegovina',
  'Botswana',
  'Bouvet Island',
  'Brazil',
  'British Indian Ocean Territory',
  'Brunei Darussalam',
  'Bulgaria',
  'Burkina Faso',
  'Burundi',
  'Cambodia',
  'Cameroon',
  'Canada',
  'Cape Verde',
  'Cayman Islands',
  'Central African Republic',
  'Chad',
  'Chile',
  'China',
  'Christmas Island',
  'Cocos (Keeling) Islands',
  'Colombia',
  'Comoros',
  'Congo',
  'Congo, Democratic Republic of the Congo',
  'Cook Islands',
  'Costa Rica',
  'Cote D\'Ivoire',
  'Croatia',
  'Cuba',
  'Curacao',
  'Cyprus',
  'Czech Republic',
  'Denmark',
  'Djibouti',
  'Dominica',
  'Dominican Republic',
  'Ecuador',
  'Egypt',
  'El Salvador',
  'Equatorial Guinea',
  'Eritrea',
  'Estonia',
  'Ethiopia',
  'Falkland Islands (Malvinas)',
  'Faroe Islands',
  'Fiji',
  'Finland',
  'France',
  'French Guiana',
  'French Polynesia',
  'French Southern Territories',
  'Gabon',
  'Gambia',
  'Georgia',
  'Germany',
  'Ghana',
  'Gibraltar',
  'Greece',
  'Greenland',
  'Grenada',
  'Guadeloupe',
  'Guam',
  'Guatemala',
  'Guernsey',
  'Guinea',
  'Guinea-Bissau',
  'Guyana',
  'Haiti',
  'Heard Island and McDonald Islands',
  'Holy See (Vatican City State)',
  'Honduras',
  'Hong Kong',
  'Hungary',
  'Iceland',
  'India',
  'Indonesia',
  'Iraq',
  'Ireland',
  'Isle of Man',
  'Israel',
  'Italy',
  'Jamaica',
  'Japan',
  'Jersey',
  'Jordan',
  'Kazakhstan',
  'Kenya',
  'Kiribati',
  'Іщгер Korea',
  'Kosovo',
  'Kuwait',
  'Kyrgyzstan',
  'Lao People\'s Democratic Republic',
  'Latvia',
  'Lebanon',
  'Lesotho',
  'Liberia',
  'Libyan Arab Jamahiriya',
  'Liechtenstein',
  'Lithuania',
  'Luxembourg',
  'Macao',
  'Macedonia, the Former Yugoslav Republic of',
  'Madagascar',
  'Malawi',
  'Malaysia',
  'Maldives',
  'Mali',
  'Malta',
  'Marshall Islands',
  'Martinique',
  'Mauritania',
  'Mauritius',
  'Mayotte',
  'Mexico',
  'Micronesia, Federated States of',
  'Moldova, Republic of',
  'Monaco',
  'Mongolia',
  'Montenegro',
  'Montserrat',
  'Morocco',
  'Mozambique',
  'Myanmar',
  'Namibia',
  'Nauru',
  'Nepal',
  'Netherlands',
  'Netherlands Antilles',
  'New Caledonia',
  'New Zealand',
  'Nicaragua',
  'Niger',
  'Nigeria',
  'Niue',
  'Norfolk Island',
  'Northern Mariana Islands',
  'Norway',
  'Oman',
  'Pakistan',
  'Palau',
  'Palestinian Territory, Occupied',
  'Panama',
  'Papua New Guinea',
  'Paraguay',
  'Peru',
  'Philippines',
  'Pitcairn',
  'Poland',
  'Portugal',
  'Puerto Rico',
  'Qatar',
  'Reunion',
  'Romania',
  'Rwanda',
  'Saint Barthelemy',
  'Saint Helena',
  'Saint Kitts and Nevis',
  'Saint Lucia',
  'Saint Martin',
  'Saint Pierre and Miquelon',
  'Saint Vincent and the Grenadines',
  'Samoa',
  'San Marino',
  'Sao Tome and Principe',
  'Saudi Arabia',
  'Senegal',
  'Serbia',
  'Serbia and Montenegro',
  'Seychelles',
  'Sierra Leone',
  'Singapore',
  'St Martin',
  'Slovakia',
  'Slovenia',
  'Solomon Islands',
  'Somalia',
  'South Africa',
  'South Georgia and the South Sandwich Islands',
  'South Sudan',
  'Spain',
  'Sri Lanka',
  'Sudan',
  'Suriname',
  'Svalbard and Jan Mayen',
  'Swaziland',
  'Sweden',
  'Switzerland',
  'Syrian Arab Republic',
  'Taiwan, Province of China',
  'Tajikistan',
  'Tanzania, United Republic of',
  'Thailand',
  'Timor-Leste',
  'Togo',
  'Tokelau',
  'Tonga',
  'Trinidad and Tobago',
  'Tunisia',
  'Turkey',
  'Turkmenistan',
  'Turks and Caicos Islands',
  'Tuvalu',
  'Uganda',
  'Ukraine',
  'United Arab Emirates',
  'United Kingdom',
  'United States',
  'United States Minor Outlying Islands',
  'Uruguay',
  'Uzbekistan',
  'Vanuatu',
  'Venezuela',
  'Viet Nam',
  'Virgin Islands, British',
  'Virgin Islands, U.s.',
  'Wallis and Futuna',
  'Western Sahara',
  'Yemen',
  'Zambia',
  'Zimbabwe',
];
const languages = () => [
  'Afrikaans',
  'Akan',
  'Albanian',
  'Amharic',
  'Arabic',
  'Aragonese',
  'Armenian',
  'Assamese',
  'Avaric',
  'Avestan',
  'Aymara',
  'Azerbaijani',
  'Bambara',
  'Bashkir',
  'Basque',
  'Belarusian',
  'Bengali',
  'Bihari languages',
  'Bislama',
  'Bosnian',
  'Breton',
  'Bulgarian',
  'Catalan',
  'Central Khmer',
  'Chamorro',
  'Chechen',
  'Chichewa',
  'Chinese',
  'Chuvash',
  'Corsican',
  'Cree',
  'Croatian',
  'Czech',
  'Danish',
  'Dutch',
  'Dzongkha',
  'English',
  'Esperanto',
  'Estonian',
  'Ewe',
  'Faroese',
  'Fijian',
  'Finnish',
  'French',
  'Fulah',
  'Galician',
  'Ganda',
  'Georgian',
  'German',
  'Greek',
  'Greenlandic',
  'Guarani',
  'Gujarati',
  'Haitian',
  'Hausa',
  'Hebrew',
  'Herero',
  'Hindi',
  'Hiri Motu',
  'Hungarian',
  'Icelandic',
  'Ido',
  'Igbo',
  'Indonesian',
  'Interlingua',
  'Interlingue',
  'Inuktitut',
  'Inupiaq',
  'Irish',
  'Italian',
  'Japanese',
  'Javanese',
  'Kalaallisut',
  'Kannada',
  'Kanuri',
  'Kashmiri',
  'Kazakh',
  'Kikuyu',
  'Kinyarwanda',
  'Kirghiz',
  'Kongo',
  'Korean',
  'Kuanyama',
  'Kurdish',
  'Lao',
  'Latin',
  'Latvian',
  'Limburgan',
  'Lingala',
  'Lithuanian',
  'Luba-Katanga',
  'Luxembourgish',
  'Macedonian',
  'Malagasy',
  'Malay',
  'Malayalam',
  'Maldivian',
  'Maltese',
  'Manx',
  'Maori',
  'Marathi',
  'Marshallese',
  'Mongolian',
  'Nauru',
  'Navajo',
  'Nepali',
  'Ndonga',
  'North Ndebele',
  'Northern Sami',
  'Norwegian',
  'Norwegian Bokmål',
  'Norwegian Nynorsk',
  'Occitan',
  'Ojibwa',
  'Oriya',
  'Oromo',
  'Ossetic',
  'Pali',
  'Panjabi',
  'Persian',
  'Polish',
  'Portuguese',
  'Pushto',
  'Quechua',
  'Romanian',
  'Romansh',
  'Rundi',
  'Russian',
  'Samoan',
  'Sardinian',
  'Scots Gaelic',
  'Serbian',
  'Serbo-Croatian',
  'Sesotho',
  'Shona',
  'Sichuan Yi',
  'Sindhi',
  'Sinhala',
  'Slovak',
  'Slovenian',
  'Somali',
  'Sotho, Southern',
  'Spanish',
  'Sundanese',
  'Swahili',
  'Swati',
  'Swedish',
  'Tagalog',
  'Tahitian',
  'Tajik',
  'Tamil',
  'Tatar',
  'Telugu',
  'Thai',
  'Tibetan',
  'Tigrinya',
  'Tonga',
  'Tswana',
  'Turkish',
  'Turkmen',
  'Twi',
  'Uighur',
  'Ukrainian',
  'Urdu',
  'Uzbek',
  'Venda',
  'Vietnamese',
  'Volapük',
  'Walloon',
  'Welsh',
  'Western Frisian',
  'Wolof',
  'Xhosa',
  'Yiddish',
  'Yoruba',
  'Zhuang',
  'Zulu',
];
const availableOptions = () => [
  { text: 'ASAP', level: 0 },
  { text: '1 week', level: 1 },
  { text: '2 weeks', level: 2 },
  { text: '3 weeks', level: 3 },
  { text: 'Month+', level: 4 },
];
const skills = () => [
  'JavaScript',
  'Python',
  'Java',
  'Ruby',
  'C#',
  'PHP',
  'HTML',
  'CSS',
  'Swift',
  'Kotlin',
  'Objective-C',
  'SQL',
  'Go',
  'C++',
  'TypeScript',
  'Rust',
  'Perl',
  'Scala',
  'Haskell',
  'Lua',
  'React',
  'Angular',
  'Vue.js',
  'Ember.js',
  'Backbone.js',
  'Sass',
  'Less',
  'jQuery',
  'Redux',
  'D3.js',
  'Three.js',
  'Bootstrap',
  'Foundation',
  'Spring Boot',
  'Django',
  'Ruby on Rails',
  'Express.js',
  'Flask',
  '.NET Core',
  'Laravel',
  'Symfony',
  'ASP.NET',
  'Node.js',
  'Dropwizard',
  'React Native',
  'Flutter',
  'Xamarin',
  'Ionic',
  'PhoneGap',
  'NativeScript',
  'Docker',
  'Kubernetes',
  'Jenkins',
  'Travis CI',
  'Ansible',
  'Terraform',
  'Puppet',
  'Chef',
  'Vagrant',
  'Git',
  'GitLab CI/CD',
  'Selenium',
  'Appium',
  'JIRA',
  'TestRail',
  'Cypress',
  'Postman',
  'JUnit',
  'TestNG',
  'Robot Framework',
  'Mocha',
  'MySQL',
  'PostgreSQL',
  'Supabase',
  'Prisma',
  'ORM',
  'MongoDB',
  'Oracle Database',
  'SQL Server',
  'Redis',
  'Cassandra',
  'Firebase',
  'Elasticsearch',
  'Neo4j',
  'AWS',
  'Azure',
  'Google Cloud',
  'Heroku',
  'DigitalOcean',
  'IBM Cloud',
  'Alibaba Cloud',
  'Oracle Cloud',
  'Kubernetes Engine',
  'Lambda',
  'R',
  'MATLAB',
  'TensorFlow',
  'PyTorch',
  'Scikit-learn',
  'Pandas',
  'Jupyter',
  'Tableau',
  'Power BI',
  'BigML',
  'Bitbucket',
  'Jira',
  'Confluence',
  'Slack',
  'Trello',
  'Asana',
  'Notion',
  'Figma',
  'Sketch',
  'Nmap',
  'Wireshark',
  'Metasploit',
  'Snort',
  'OpenVAS',
  'Hashcat',
  'OWASP ZAP',
  'Kali Linux',
  'Splunk',
  'Qualys',
  'CircleCI',
  'TeamCity',
  'Bamboo',
  'GitLab CI',
  'Travis CI',
  'Codeship',
  'GoCD',
  'Octopus Deploy',
  'Capistrano',
  'Jenkins X',
  'Rust',
  'Perl',
  'Scala',
  'Haskell',
  'Lua',
  'Dart',
  'Groovy',
  'Elixir',
  'Clojure',
  'COBOL',
  'F#',
  'Ada',
  'PL/SQL',
  'Swift',
  'Kotlin',
  'Objective-C',
  'Racket',
  'Scheme',
  'Matlab',
  'Julia',
  'Next.js',
  'Nuxt.js',
  'Gatsby',
  'Tailwind CSS',
  'Material-UI',
  'Ant Design',
  'Foundation',
  'Bulma',
  'Semantic UI',
  'Materialize',
  'Quasar',
  'Chakra UI',
  'Buefy',
  'Carbon Design System',
  'Vuetify',
  'Element UI',
  'Dojo Toolkit',
  'Mithril',
  'Play Framework',
  'Nest.js',
  'AdonisJS',
  'Phoenix',
  'Grails',
  'Ratpack',
  'Sinatra',
  'Hanami',
  'Sails.js',
  'LoopBack',
  'Nest.js',
  'Feathers',
  'CakePHP',
  'Slim',
  'Rocket',
  'Warp',
  'Corona SDK',
  'Sencha Touch',
  'Mobile Angular UI',
  'Kivy',
  'Codename One',
  'Nativescript-Vue',
  'Fuse',
  'Appgyver',
  'Red Hat Mobile',
  'Xojo',
  'JUCE',
  'Xamarin.Forms',
  'AppSheet',
  'Nomad',
  'Consul',
  'Spinnaker',
  'Rancher',
  'Rundeck',
  'JFrog Artifactory',
  'Harbor',
  'Packer',
  'Nomad',
  'Rook',
  'Helm',
  'Telegraf',
  'Fluentd',
  'Prometheus',
  'Grafana',
  'Zipkin',
  'Jest',
  'Enzyme',
  'Detox',
  'Gauge',
  'Watir',
  'Katalon Studio',
  'Gatling',
  'Locust',
  'Karate',
  'Geb',
  'Behat',
  'Cucumber',
  'Spock',
  'Galen',
  'SQLite',
  'MariaDB',
  'Couchbase',
  'Amazon Aurora',
  'DB2',
  'Teradata',
  'HBase',
  'CouchDB',
  'ArangoDB',
  'Riak',
  'InfluxDB',
  'Snowflake',
  'CockroachDB',
  'Amazon Redshift',
  'Memcached',
  'Hive',
  'Erlang',
  'COBOL',
  'F#',
  'Ada',
  'PL/SQL',
  'Swift',
  'Kotlin',
  'Objective-C',
  'Racket',
  'Scheme',
  'Matlab',
  'Julia',
  'Fortran',
  'Prolog',
  'Dart',
  'Rexx',
  'Verilog',
  'VHDL',
  'COOL',
  'FoxPro',
  'Tcl',
  'COOL',
  'Apex',
  'Logtalk',
  'RPG',
  'XQuery',
  'ABAP',
  'IDL',
  'Forth',
  'MUMPS',
  'PL/I',
  'Oberon',
  'Logo',
  'SAS',
  'Simula',
  'Scratch',
  'Smalltalk',
  'Eiffel',
  'COOL',
  'Oxygene',
  'R',
  'Hack',
  'Alice',
  'Dart',
  'Elm',
  'Red',
  'PowerShell',
  'Rust',
  'Julia',
  'Svelte',
  'Stencil',
  'Quasar',
  'Chakra UI',
  'Buefy',
  'Carbon Design System',
  'Vuetify',
  'Element UI',
  'Dojo Toolkit',
  'Mithril',
  'Aurelia',
  'Polymer',
  'Knockout.js',
  'Riot.js',
  'MooTools',
  'Ext JS',
  'Zurb Foundation',
  'BEM',
  'Pure.css',
  'Material Design Lite',
  'Semantic UI React',
  'Onsen UI',
  'Weex',
  'AMP (Accelerated Mobile Pages)',
  'BonsaiJS',
  'Preact',
  'Alpine.js',
  'Moon.js',
  'Stimulus',
  'SproutCore',
  'Play Framework',
  'Nest.js',
  'AdonisJS',
  'Phoenix',
  'Grails',
  'Ratpack',
  'Sinatra',
  'Hanami',
  'Sails.js',
  'LoopBack',
  'Feathers',
  'CakePHP',
  'Slim',
  'Rocket',
  'Warp',
  'Vert.x',
  'Actix',
  'Nancy',
  'Kitura',
  'Perfect',
  'Zewo',
  'Rails API',
  'Lumen',
  'Hug',
  'Buffalo',
  'Flask-RESTful',
  'Falcon',
  'Slim Framework',
  'AppSheet',
  'Unity',
  'Cocos2d',
  'Lumberyard',
  'Defold',
  'Buildbox',
  'GameMaker',
  'AppMakr',
  'GameSalad',
  'Unreal Engine',
  'Gideros',
  'Moai',
  'CryEngine',
  'Oxygine',
  'Stencyl',
  'Phaser',
  'Godot',
  'Marmalade',
  'Monogame',
  'Cordova',
];
const englishLevels = () => [{ text: 'Basic (A0-A2)', level: 0 },
  { text: 'Intermediate (B1)', level: 1 },
  { text: 'Upper Intermediate (B2)', level: 2 },
  { text: 'Fluent (C1-C2)', level: 3 },
  { text: 'Native / Bilingual', level: 4 }];

const roles = () => [
  {
    role: 'Frontend Developer',
    description: 'Specializes in creating user interfaces and client-side development using technologies like HTML, CSS, and JavaScript.',
  },
  {
    role: 'Backend Developer',
    description: 'Focuses on server-side development, database management, and server operations.',
  },
  {
    role: 'Full-Stack Developer',
    description: 'Proficient in both frontend and backend development, capable of working on the entire software stack.',
  },
  {
    role: 'Mobile App Developer',
    description: 'Specializes in creating applications for mobile platforms such as iOS and Android.',
  },
  {
    role: 'DevOps Engineer',
    description: 'Manages the development, deployment, and maintenance of software, focusing on automation, continuous integration, and continuous deployment (CI/CD).',
  },
  {
    role: 'Manual QA Engineer',
    description: 'Ensures the quality and functionality of software through testing and quality control processes.',
  },
  {
    role: 'QA Automation Engineer',
    description: 'Develops automated scripts and tools to streamline development and testing processes.',
  },
  {
    role: 'Software Architect',
    description: 'Designs the high-level structure of software systems, defining key components and their interactions.',
  },
  {
    role: 'UI/UX Designer',
    description: 'Focuses on creating intuitive and visually appealing user interfaces and user experiences.',
  },
  {
    role: 'Product Manager',
    description: 'Oversees the development process, defines product goals, and prioritizes features based on customer and business needs.',
  },
  {
    role: 'Scrum Master',
    description: 'Facilitates agile development processes, ensuring the team follows Scrum practices and removing obstacles.',
  },
  {
    role: 'Business Analyst',
    description: 'Analyzes business requirements and translates them into software requirements and features.',
  },
  {
    role: 'Database Administrator (DBA)',
    description: 'Manages and maintains the company\'s databases, ensuring data integrity and availability.',
  },
  {
    role: 'Systems Administrator',
    description: 'Handles server infrastructure, network configurations, and system maintenance.',
  },
  {
    role: 'Security Engineer',
    description: 'Focuses on the security of software and infrastructure, implementing security measures and conducting security audits.',
  },
  {
    role: 'Cloud Engineer',
    description: 'Manages cloud infrastructure and services, such as AWS, Azure, or Google Cloud.',
  },
  {
    role: 'Data Scientist',
    description: 'Analyzes and interprets complex data to provide insights and make data-driven decisions.',
  },
  {
    role: 'Machine Learning Engineer',
    description: 'Develops machine learning models and algorithms for data analysis and predictive purposes.',
  },
  {
    role: 'Technical Writer',
    description: 'Creates documentation, user guides, and manuals for software products.',
  },
  {
    role: 'Support Engineer',
    description: 'Provides technical support to customers, addressing their issues and inquiries.',
  },
  {
    role: 'Release Manager',
    description: 'Oversees the release process, ensuring smooth deployments and version control.',
  },
  {
    role: 'Project Manager',
    description: 'Manages software development projects, including scheduling, budgeting, and resource allocation.',
  },
  {
    role: 'Data Engineer',
    description: 'Focuses on the design and management of data pipelines and ETL processes.',
  },
  {
    role: 'Release Engineer',
    description: 'Manages the release process, including build, packaging, and deployment of software.',
  },
  {
    role: 'Site Reliability Engineer (SRE)',
    description: 'Focuses on the reliability, availability, and performance of software and systems in production.',
  },
  {
    role: 'Other Role',
    description: '',
  },
];

console.log(roles);


export { countries, skills, availableOptions, englishLevels, languages };

const countriesArray = [
  { country: 'Andorra', region: 'Europe' },
  { country: 'United Arab Emirates', region: 'Asia' },
  { country: 'Afghanistan', region: 'Asia' },
  { country: 'Antigua and Barbuda', region: 'North America' },
  { country: 'Anguilla', region: 'North America' },
  { country: 'Albania', region: 'Europe' },
  { country: 'Armenia', region: 'Asia' },
  { country: 'Angola', region: 'Africa' },
  { country: 'Antarctica', region: 'Antarctica' },
  { country: 'Argentina', region: 'South America' },
  { country: 'American Samoa', region: 'Oceania' },
  { country: 'Austria', region: 'Europe' },
  { country: 'Australia', region: 'Oceania' },
  { country: 'Aruba', region: 'North America' },
  { country: 'Åland', region: 'Europe' },
  { country: 'Azerbaijan', region: 'Asia' },
  { country: 'Bosnia and Herzegovina', region: 'Europe' },
  { country: 'Barbados', region: 'North America' },
  { country: 'Bangladesh', region: 'Asia' },
  { country: 'Belgium', region: 'Europe' },
  { country: 'Burkina Faso', region: 'Africa' },
  { country: 'Bulgaria', region: 'Europe' },
  { country: 'Bahrain', region: 'Asia' },
  { country: 'Burundi', region: 'Africa' },
  { country: 'Benin', region: 'Africa' },
  { country: 'Saint Barthélemy', region: 'North America' },
  { country: 'Bermuda', region: 'North America' },
  { country: 'Brunei', region: 'Asia' },
  { country: 'Bolivia', region: 'South America' },
  { country: 'Bonaire', region: 'North America' },
  { country: 'Brazil', region: 'South America' },
  { country: 'Bahamas', region: 'North America' },
  { country: 'Bhutan', region: 'Asia' },
  { country: 'Bouvet Island', region: 'Antarctica' },
  { country: 'Botswana', region: 'Africa' },
  { country: 'Belarus', region: 'Europe' },
  { country: 'Belize', region: 'North America' },
  { country: 'Canada', region: 'North America' },
  { country: 'Cocos [Keeling] Islands', region: 'Asia' },
  { country: 'Democratic Republic of the Congo', region: 'Africa' },
  { country: 'Central African Republic', region: 'Africa' },
  { country: 'Republic of the Congo', region: 'Africa' },
  { country: 'Switzerland', region: 'Europe' },
  { country: 'Ivory Coast', region: 'Africa' },
  { country: 'Cook Islands', region: 'Oceania' },
  { country: 'Chile', region: 'South America' },
  { country: 'Cameroon', region: 'Africa' },
  { country: 'China', region: 'Asia' },
  { country: 'Colombia', region: 'South America' },
  { country: 'Costa Rica', region: 'North America' },
  { country: 'Cuba', region: 'North America' },
  { country: 'Cape Verde', region: 'Africa' },
  { country: 'Curacao', region: 'North America' },
  { country: 'Christmas Island', region: 'Asia' },
  { country: 'Cyprus', region: 'Europe' },
  { country: 'Czechia', region: 'Europe' },
  { country: 'Germany', region: 'Europe' },
  { country: 'Djibouti', region: 'Africa' },
  { country: 'Denmark', region: 'Europe' },
  { country: 'Dominica', region: 'North America' },
  { country: 'Dominican Republic', region: 'North America' },
  { country: 'Algeria', region: 'Africa' },
  { country: 'Ecuador', region: 'South America' },
  { country: 'Estonia', region: 'Europe' },
  { country: 'Egypt', region: 'Africa' },
  { country: 'Western Sahara', region: 'Africa' },
  { country: 'Eritrea', region: 'Africa' },
  { country: 'Spain', region: 'Europe' },
  { country: 'Ethiopia', region: 'Africa' },
  { country: 'Finland', region: 'Europe' },
  { country: 'Fiji', region: 'Oceania' },
  { country: 'Falkland Islands', region: 'South America' },
  { country: 'Micronesia', region: 'Oceania' },
  { country: 'Faroe Islands', region: 'Europe' },
  { country: 'France', region: 'Europe' },
  { country: 'Gabon', region: 'Africa' },
  { country: 'United Kingdom', region: 'Europe' },
  { country: 'Grenada', region: 'North America' },
  { country: 'Georgia', region: 'Asia' },
  { country: 'French Guiana', region: 'South America' },
  { country: 'Guernsey', region: 'Europe' },
  { country: 'Ghana', region: 'Africa' },
  { country: 'Gibraltar', region: 'Europe' },
  { country: 'Greenland', region: 'North America' },
  { country: 'Gambia', region: 'Africa' },
  { country: 'Guinea', region: 'Africa' },
  { country: 'Guadeloupe', region: 'North America' },
  { country: 'Equatorial Guinea', region: 'Africa' },
  { country: 'Greece', region: 'Europe' },
  { country: 'South Georgia and the South Sandwich Islands', region: 'Antarctica' },
  { country: 'Guatemala', region: 'North America' },
  { country: 'Guam', region: 'Oceania' },
  { country: 'Guinea-Bissau', region: 'Africa' },
  { country: 'Guyana', region: 'South America' },
  { country: 'Hong Kong', region: 'Asia' },
  { country: 'Heard Island and McDonald Islands', region: 'Antarctica' },
  { country: 'Honduras', region: 'North America' },
  { country: 'Croatia', region: 'Europe' },
  { country: 'Haiti', region: 'North America' },
  { country: 'Hungary', region: 'Europe' },
  { country: 'Indonesia', region: 'Asia' },
  { country: 'Ireland', region: 'Europe' },
  { country: 'Israel', region: 'Asia' },
  { country: 'Isle of Man', region: 'Europe' },
  { country: 'India', region: 'Asia' },
  { country: 'British Indian Ocean Territory', region: 'Asia' },
  { country: 'Iraq', region: 'Asia' },
  { country: 'Iran', region: 'Asia' },
  { country: 'Iceland', region: 'Europe' },
  { country: 'Italy', region: 'Europe' },
  { country: 'Jersey', region: 'Europe' },
  { country: 'Jamaica', region: 'North America' },
  { country: 'Jordan', region: 'Asia' },
  { country: 'Japan', region: 'Asia' },
  { country: 'Kenya', region: 'Africa' },
  { country: 'Kyrgyzstan', region: 'Asia' },
  { country: 'Cambodia', region: 'Asia' },
  { country: 'Kiribati', region: 'Oceania' },
  { country: 'Comoros', region: 'Africa' },
  { country: 'Saint Kitts and Nevis', region: 'North America' },
  { country: 'North Korea', region: 'Asia' },
  { country: 'South Korea', region: 'Asia' },
  { country: 'Kuwait', region: 'Asia' },
  { country: 'Cayman Islands', region: 'North America' },
  { country: 'Kazakhstan', region: 'Asia' },
  { country: 'Laos', region: 'Asia' },
  { country: 'Lebanon', region: 'Asia' },
  { country: 'Saint Lucia', region: 'North America' },
  { country: 'Liechtenstein', region: 'Europe' },
  { country: 'Sri Lanka', region: 'Asia' },
  { country: 'Liberia', region: 'Africa' },
  { country: 'Lesotho', region: 'Africa' },
  { country: 'Lithuania', region: 'Europe' },
  { country: 'Luxembourg', region: 'Europe' },
  { country: 'Latvia', region: 'Europe' },
  { country: 'Libya', region: 'Africa' },
  { country: 'Morocco', region: 'Africa' },
  { country: 'Monaco', region: 'Europe' },
  { country: 'Moldova', region: 'Europe' },
  { country: 'Montenegro', region: 'Europe' },
  { country: 'Saint Martin', region: 'North America' },
  { country: 'Madagascar', region: 'Africa' },
  { country: 'Marshall Islands', region: 'Oceania' },
  { country: 'Macedonia', region: 'Europe' },
  { country: 'Mali', region: 'Africa' },
  { country: 'Myanmar [Burma]', region: 'Asia' },
  { country: 'Mongolia', region: 'Asia' },
  { country: 'Macao', region: 'Asia' },
  { country: 'Northern Mariana Islands', region: 'Oceania' },
  { country: 'Martinique', region: 'North America' },
  { country: 'Mauritania', region: 'Africa' },
  { country: 'Montserrat', region: 'North America' },
  { country: 'Malta', region: 'Europe' },
  { country: 'Mauritius', region: 'Africa' },
  { country: 'Maldives', region: 'Asia' },
  { country: 'Malawi', region: 'Africa' },
  { country: 'Mexico', region: 'North America' },
  { country: 'Malaysia', region: 'Asia' },
  { country: 'Mozambique', region: 'Africa' },
  { country: 'Namibia', region: 'Africa' },
  { country: 'New Caledonia', region: 'Oceania' },
  { country: 'Niger', region: 'Africa' },
  { country: 'Norfolk Island', region: 'Oceania' },
  { country: 'Nigeria', region: 'Africa' },
  { country: 'Nicaragua', region: 'North America' },
  { country: 'Netherlands', region: 'Europe' },
  { country: 'Norway', region: 'Europe' },
  { country: 'Nepal', region: 'Asia' },
  { country: 'Nauru', region: 'Oceania' },
  { country: 'Niue', region: 'Oceania' },
  { country: 'New Zealand', region: 'Oceania' },
  { country: 'Oman', region: 'Asia' },
  { country: 'Panama', region: 'North America' },
  { country: 'Peru', region: 'South America' },
  { country: 'French Polynesia', region: 'Oceania' },
  { country: 'Papua New Guinea', region: 'Oceania' },
  { country: 'Philippines', region: 'Asia' },
  { country: 'Pakistan', region: 'Asia' },
  { country: 'Poland', region: 'Europe' },
  { country: 'Saint Pierre and Miquelon', region: 'North America' },
  { country: 'Pitcairn Islands', region: 'Oceania' },
  { country: 'Puerto Rico', region: 'North America' },
  { country: 'Palestine', region: 'Asia' },
  { country: 'Portugal', region: 'Europe' },
  { country: 'Palau', region: 'Oceania' },
  { country: 'Paraguay', region: 'South America' },
  { country: 'Qatar', region: 'Asia' },
  { country: 'Réunion', region: 'Africa' },
  { country: 'Romania', region: 'Europe' },
  { country: 'Serbia', region: 'Europe' },
  { country: 'Russia', region: 'Europe' },
  { country: 'Rwanda', region: 'Africa' },
  { country: 'Saudi Arabia', region: 'Asia' },
  { country: 'Solomon Islands', region: 'Oceania' },
  { country: 'Seychelles', region: 'Africa' },
  { country: 'Sudan', region: 'Africa' },
  { country: 'Sweden', region: 'Europe' },
  { country: 'Singapore', region: 'Asia' },
  { country: 'Saint Helena', region: 'Africa' },
  { country: 'Slovenia', region: 'Europe' },
  { country: 'Svalbard and Jan Mayen', region: 'Europe' },
  { country: 'Slovakia', region: 'Europe' },
  { country: 'Sierra Leone', region: 'Africa' },
  { country: 'San Marino', region: 'Europe' },
  { country: 'Senegal', region: 'Africa' },
  { country: 'Somalia', region: 'Africa' },
  { country: 'Suriname', region: 'South America' },
  { country: 'South Sudan', region: 'Africa' },
  { country: 'São Tomé and Príncipe', region: 'Africa' },
  { country: 'El Salvador', region: 'North America' },
  { country: 'Sint Maarten', region: 'North America' },
  { country: 'Syria', region: 'Asia' },
  { country: 'Swaziland', region: 'Africa' },
  { country: 'Turks and Caicos Islands', region: 'North America' },
  { country: 'Chad', region: 'Africa' },
  { country: 'French Southern Territories', region: 'Antarctica' },
  { country: 'Togo', region: 'Africa' },
  { country: 'Thailand', region: 'Asia' },
  { country: 'Tajikistan', region: 'Asia' },
  { country: 'Tokelau', region: 'Oceania' },
  { country: 'East Timor', region: 'Oceania' },
  { country: 'Turkmenistan', region: 'Asia' },
  { country: 'Tunisia', region: 'Africa' },
  { country: 'Tonga', region: 'Oceania' },
  { country: 'Turkey', region: 'Asia' },
  { country: 'Trinidad and Tobago', region: 'North America' },
  { country: 'Tuvalu', region: 'Oceania' },
  { country: 'Taiwan', region: 'Asia' },
  { country: 'Tanzania', region: 'Africa' },
  { country: 'Ukraine', region: 'Europe' },
  { country: 'Uganda', region: 'Africa' },
  { country: 'U.S. Minor Outlying Islands', region: 'Oceania' },
  { country: 'United States', region: 'North America' },
  { country: 'Uruguay', region: 'South America' },
  { country: 'Uzbekistan', region: 'Asia' },
  { country: 'Vatican City', region: 'Europe' },
  { country: 'Saint Vincent and the Grenadines', region: 'North America' },
  { country: 'Venezuela', region: 'South America' },
  { country: 'British Virgin Islands', region: 'North America' },
  { country: 'U.S. Virgin Islands', region: 'North America' },
  { country: 'Vietnam', region: 'Asia' },
  { country: 'Vanuatu', region: 'Oceania' },
  { country: 'Wallis and Futuna', region: 'Oceania' },
  { country: 'Samoa', region: 'Oceania' },
  { country: 'Kosovo', region: 'Europe' },
  { country: 'Yemen', region: 'Asia' },
  { country: 'Mayotte', region: 'Africa' },
  { country: 'South Africa', region: 'Africa' },
  { country: 'Zambia', region: 'Africa' },
  { country: 'Zimbabwe', region: 'Africa' },
];
